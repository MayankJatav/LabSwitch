//====================================================================================================//
	...
	HANDLE hComm;                          // Handle to the Serial port
			...
			char  SerialBuffer[100],s[10],com[10];               // Buffer Containing Rxed Data
			DWORD NoBytesRead;                     // Bytes read by ReadFile()
			int i = 0, j=0;
    /*char *a[]={0,0,0,0,0};
	void tok(char str[])//Returns the String
	{
        int i=0;*/
        /*int j=0;
        char c;
	...
	void main(void)
		{
            ...
            else
            {
                ...
                if(s[0]!='C')
                   {
                       printf("COM port communication file might be empty or containing some corrupt data\nEnter the COM port(Syntax:- COM1, COM4, etc): ");
                       scanf("%s",&com);
                       fputs(com,fp);
                       strcat(s,com);
                    }
			...
			if (Status == FALSE)
			...
			else //If  WaitCommEvent()==True Read the RXed data using ReadFile();
				{
				    do
                    {
                        ...
                        i--;
                        int i1=0;char *a[10];
                        char uname[20],pass[20],type[20];
                        switch(SerialBuffer[0])
                        {
                        case '1':
                            sscanf(SerialBuffer,"%*d %s %s %s",uname,pass,type);
                            printf("%s %s %s",uname,pass,type);
                            serialWrite("F");
                            /*a[i1]=strtok(SerialBuffer," ");
                            while(a[i1]!=NULL)
                            {
                                a[i1]=strtok(NULL," ");
                                printf("%s ",a[i1]);
                    ...
                    while(WaitCommEvent(hComm, &dwEventMask, NULL));
				}
